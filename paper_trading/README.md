...
運転用の実行ロジックは稼働パッケージの写しです。GitHub用にモデル不在時のテストを明示的にskipする処理と、独立した合成デモを追加しました。稼働中のモデル、コード、条件、状態は変更していません。
...
依存ライブラリ導入後、ルートから `python paper_trading/demo.py` を実行できます。人工価格と固定したダミー判断で、次の観測での約定、再起動後の重複防止、3口座それぞれの決済を確認します。合成データの損益はモデルの研究成績ではありません。APIキーや配布されていないモデルを必要とせず、一時ディレクトリだけを使用します。
